        ...
        if code != 0:
            ...
            for i in data:
                for w in i["cw"]:
                    result += w["w"]
            # 初始化翻译里面的get_result类
            its_req = get_result(result)
            # 调用get_result类里面的翻译请求方法： call_url()
            trans_result = its_req.call_url()
            print('原始文本结果--->', result)
            print('翻译后的结果--->', trans_result)
...
if __name__ == "__main__":
    ...
    wsParam = Ws_Param(APPID='',
                       APISecret='',
                       APIKey='',
                       AudioFile=r'C:\Users\yanmeng2\Desktop\1.pcm')
